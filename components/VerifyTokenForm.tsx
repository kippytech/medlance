  ...
  async function onSubmit(data: z.infer<typeof FormSchema>) {
    ...
    if (userInputToken === userToken) {
      ...
      try {
        ...
        setLoading(false);
        toast.error("Encountered an error while verifying");
        console.log(error);
      }
    } else {
      ...
      <form onSubmit={form.handleSubmit(onSubmit)} className="w-2/3 space-y-6">
        {showNotification && (
          <Alert color="failure" icon={HiInformationCircle}>
            <span className="font-medium">Wrong Pass code!</span> Please Check the
            pass code and Enter again
          </Alert>
        )}
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              <FormLabel>Enter the Pass code here</FormLabel>
              <FormControl>
